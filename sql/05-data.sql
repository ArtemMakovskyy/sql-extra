...
SELECT
    acc_c.account_created + es.sent_date AS sent_date,
    acc_c.account_created + eo.open_date AS open_date,
    acc_c.account_created + ev.visit_date AS visit_date,
    acc.id AS id_account,
    es.id_message
FROM account acc
         JOIN acc_c ON acc.id = acc_c.account_id
